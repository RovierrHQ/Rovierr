/** biome-ignore-all lint/performance/useTopLevelRegex: from nativewindui */
...
function withOpacityHex(hex: string, opacity: number): string {
  ...
  const r = Number.parseInt(expandedHex.substr(0, 2), 16)
  const g = Number.parseInt(expandedHex.substr(2, 2), 16)
  const b = Number.parseInt(expandedHex.substr(4, 2), 16)
