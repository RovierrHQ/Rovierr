...
export const studentOrganizationsRouter = new Elysia({
  prefix: '/student-organizations'
}).get('/list-all-organizations', async ({ query, set }) => {
  try {
    ...
    const userInstitutionId: string | null = null
    // Note: In Elysia, you'd need to add auth middleware to get session
    // For now, we'll proceed without user context
    ...
    return {
      data: organizations.map((org) => ({
        ...
        visibility: org.visibility as 'public' | 'campus_only' | 'private',
