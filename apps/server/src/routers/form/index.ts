...
export const form = new Elysia({ prefix: '/form' })
	...
	.group('', { auth: true }, (app) =>
		app
			...
			.patch('/update', async ({ body, user }) => {
				if (!user) throw new Error('User not authenticated')
				try {
					return await formService.updateForm(body.id, user.id, body)
				} catch (error) {
					if (error instanceof Error) {
						if (error.message === 'Form not found') throw new Error('Form not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to edit this form')
					}
					throw error
				}
			...
			.get('/list', async ({ query, user }) => {
				if (!user) throw new Error('User not authenticated')
				const result = await formService.listForms({ ...query, userId: user.id })
				return {
			...
			.delete('/:id', async ({ params, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Form not found') throw new Error('Form not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to delete this form')
					}
					throw error
				}
			})
			...
			.post('/:id/publish', async ({ params, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Form not found') throw new Error('Form not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to publish this form')
					}
					throw error
				}
			})
			...
			.post('/:id/unpublish', async ({ params, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Form not found') throw new Error('Form not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to unpublish this form')
					}
					throw error
				}
			})
			...
			.post('/:id/duplicate', async ({ params, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Form not found') throw new Error('Form not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to duplicate this form')
					}
					throw error
				}
			})
			...
			.post('/:id/archive', async ({ params, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Form not found') throw new Error('Form not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to archive this form')
					}
					throw error
				}
			})

			.post('/bulk-save', async ({ body, user }) => {
				if (!user) throw new Error('User not authenticated')
				try {
			...
			.post('/page/create', async ({ body, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Form not found') throw new Error('Form not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to create pages')
					}
					throw error
				}
			})
			...
			.patch('/page/:id', async ({ params, body, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Page not found') throw new Error('Page not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to update this page')
					}
					throw error
				}
			})
			...
			.delete('/page/:id', async ({ params, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Page not found') throw new Error('Page not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to delete this page')
					}
					throw error
				}
			})
			...
			.post('/question/create', async ({ body, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Page not found') throw new Error('Page not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to create questions')
					}
					throw error
				}
			})
			...
			.patch('/question/:id', async ({ params, body, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Question not found') throw new Error('Question not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to update this question')
					}
					throw error
				}
			})
			...
			.delete('/question/:id', async ({ params, user }) => {
				...
				try {
					...
					if (error instanceof Error) {
						if (error.message === 'Question not found') throw new Error('Question not found')
						if (error.message === 'Unauthorized') throw new Error('You do not have permission to delete this question')
					}
					throw error
				}
			})
			...
			.post('/question/reorder', async ({ body, user }) => {
				if (!user) throw new Error('User not authenticated')
				try {
					return await formService.reorderQuestions(body.pageId, user.id, body.questionIds)
				} catch (error) {
					if (error instanceof Error && error.message === 'Unauthorized') {
