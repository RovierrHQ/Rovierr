...
function TextField({
  ...
  const inputRef = useAugmentedRef<TextInput>({
...
const innerRootVariants = cva('flex-row rounded', {
  variants: {
    variant: {
      ...
      filled: 'bg-border rounded-b-none border-b'
    },
    state: {
  ...
  const animatedRootStyle = useAnimatedStyle(() => {
      ...
      style.transform = [
        {
          translateY: withTiming(
            isLiftedDerived.value ? -DEFAULT_TEXT_FIELD_HEIGHT / 2.2 : 0,
            {
              duration: 200
            }
          )
    ...
    if (variantDerived.value === 'filled') {
      ...
      style.transform = [
        {
          translateY: withTiming(
            isLiftedDerived.value ? -DEFAULT_TEXT_FIELD_HEIGHT / 3.75 : 0,
            {
              duration: 200
            }
          )
...
function MaterialClearIcon(props: MaterialClearIconProps) {
  ...
  return (
    ...
    >
      ...
      >
        <Icon color={colors.grey2} name="xmark.circle" />
      </Pressable>
    </Animated.View>
  )
}
