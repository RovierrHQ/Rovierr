...
import {
  ...
  ComboboxItem,
  ComboboxList
} from '@rov/ui/components/combobox'
import {
  InputGroup,
  InputGroupAddon,
  InputGroupInput
...
export function VerificationPrompt() {
  ...
  // Fetch universities from database - now public endpoint
  const {
    ...
    queryFn: async () => {
      const response = await api.university.get()
      return response
    }
  })
  ...
  // Show error state if failed to load universities
  if (universitiesError || !universitiesData) {
    return (
      <Card className="space-y-6 border-red-500/30 bg-gradient-to-br from-red-500/5 to-orange-500/5 p-6 backdrop-blur">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-muted-foreground text-sm">
              There was an error loading the university list. Please try again
              later.
            </p>
          </div>
        </div>
  ...
  // Show message if no universities available
      ...
      <Card className="space-y-6 border-amber-500/30 bg-gradient-to-br from-amber-500/5 to-orange-500/5 p-6 backdrop-blur">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-muted-foreground text-sm">
              There are no universities available for verification at this time.
